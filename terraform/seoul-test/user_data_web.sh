...
# FORCE UPDATE CHECK: WEB USER DATA
...
echo "export NEXT_PUBLIC_AWS_REGION=${aws_region}" >> /home/ec2-user/.bashrc
echo "export INTERNAL_API_URL=http://${nlb_dns_name}:3001" >> /home/ec2-user/.bashrc
chown ec2-user:ec2-user /home/ec2-user/.bashrc

# 9. Web 앱 빌드 (환경변수 inline 전달 - NEXT_PUBLIC_* 는 빌드 시점에 bake-in 됨)
echo "=== Building Web App ==="
sudo -u ec2-user bash -c "source \$HOME/.nvm/nvm.sh && cd \$HOME/megaticket && AWS_REGION=${aws_region} NEXT_PUBLIC_AWS_REGION=${aws_region} INTERNAL_API_URL=http://${nlb_dns_name}:3001 npm run build:web"
